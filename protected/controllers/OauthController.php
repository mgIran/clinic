...
class OauthController extends ApiBaseController
{
    ...
    {
        if(isset($this->request['email']) and isset($this->request['password']) && !empty($this->request['email']) && !empty($this->request['password'])){
            $login = new UserLoginForm;
            $login->scenario = 'app_login';
            $login->verification_field_value = $this->request['email'];
            ...
            $this->_sendResponse(200, CJSON::encode(['status' => false, 'message' => 'Email and Password is required.']), 'application/json');
    }
